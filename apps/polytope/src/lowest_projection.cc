										 ...
										 const bool &only_lowest)
{
	...
	if (ls)
	{											 // we can do another projection
		list<int> seq2(seq); // copy the current sequence
		seq2.push_back(c);	 // add the projection just done

		if (SQ.dim() < lowest)
		{ // if the projected polytope has a lower dimension than the currently best sequences then discard them
			lowest = SQ.dim();
			pseq.clear();
		}
		...
		{
			for (auto vpc = entire(VPC); !vpc.at_end(); ++vpc)
...
check_all_projections(perl::Object p, bool only_lowest = false, bool all_facets = false)
{
	...
	std::pair<int, std::pair<int, Array<Array<Integer>>>> pair_pseq;
	...
	else
	{
		...
		for (auto ftv_it = entire(rows(ftv)); !ftv_it.at_end(); ++ftv_it)
		{
			Matrix<Integer> MT = vector2row(unit_vector<Integer>(F.cols(), 0)) /
													 (zero_vector<Integer>(F.cols() - 1) | (-F.minor(*ftv_it, ~range(0, 0))));
			if (abs(det(MT)) != 1)
			{
				throw std::runtime_error("project_with_subdivision: transformation matrix not lattice preserving\n");
			}
			Matrix<Rational> RIT = inv(MT);
			for (auto vit = entire(concat_rows(RIT)); !vit.at_end(); ++vit)
				if (denominator(*vit) != 1)
				{
					throw std::runtime_error("project_with_subdivision: transformation matrix not lattice preserving\n");
				}
